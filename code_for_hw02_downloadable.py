...
def big_higher_dim_separable():
    X, y = gen_lin_separable(num_points=50, dim=6,
...
def test_linear_classifier(dataFun, learner, learner_params={},
    ...
    if draw:
        ...
        def hook(params):
            (th, th0) = params
            if refresh:
                plot_data(data, labels, ax, clear=True)
...
def test_eval_classifier(eval_classifier, perceptron):
    ...
    for index in range(len(dataset_train)):
        ...
        result = eval_classifier(
            perceptron, data_train, labels_train, data2, labels2)
...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    th, th0 = learner(data_train, labels_train, params={'T': 100})
    return score(data_test, labels_test, th, th0) / len(labels_test[0])
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for _ in range(it):
        ...
        scores += eval_classifier(learner, train_x, train_y, train_x, train_y)
...
def test():
    ...
    print(eval_learning_alg(averaged_perceptron,
          data_gen, n_train=20, n_test=20, it=100))
